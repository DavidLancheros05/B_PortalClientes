...
--   1. Estado cliente (sol_ses_id) — qué ve el cliente
--   2. Etapa workflow (sol_wet_id) — quién tiene la solicitud
--   3. Resultado etapa (sol_wee_id) — aprobado/rechazado en esa etapa
--
...
IF NOT EXISTS (SELECT 1
               FROM   sys.objects
               WHERE  type = 'U'
                      AND name = 'workflow_etapas')
    BEGIN
        CREATE TABLE workflow_etapas (
            wet_id         INT           IDENTITY (1, 1) PRIMARY KEY,
        ...
        CREATE INDEX idx_workflow_etapas_codigo
            ON workflow_etapas(wet_codigo);
        CREATE INDEX idx_workflow_etapas_activo
            ON workflow_etapas(wet_activo);
    END
...
IF NOT EXISTS (SELECT 1
               FROM   sys.objects
               WHERE  type = 'U'
                      AND name = 'workflow_estado_etapa')
    BEGIN
        CREATE TABLE workflow_estado_etapa (
            wee_id     INT          IDENTITY (1, 1) PRIMARY KEY,
            wee_codigo VARCHAR (10) NOT NULL UNIQUE,
            wee_nombre VARCHAR (50) NOT NULL
        );
        CREATE INDEX idx_workflow_estado_etapa_codigo
            ON workflow_estado_etapa(wee_codigo);
    END
...
IF NOT EXISTS (SELECT 1
               FROM   sys.objects
               WHERE  type = 'U'
                      AND name = 'solicitud_workflow_historial')
    BEGIN
        CREATE TABLE solicitud_workflow_historial (
            swh_id                 BIGINT         IDENTITY (1, 1) PRIMARY KEY,
        ...
        CREATE INDEX idx_swh_solicitud
            ON solicitud_workflow_historial(swh_sol_id);
        CREATE INDEX idx_swh_fecha
            ON solicitud_workflow_historial(swh_fecha);
        CREATE INDEX idx_swh_etapa
            ON solicitud_workflow_historial(swh_etapa_id);
    END
...
IF NOT EXISTS (SELECT 1
    ...
    BEGIN
        ...
        VALUES                      ('EJN', 'Ejecutivo Negocios', 1, 1),
...
IF NOT EXISTS (SELECT 1
               ...
               WHERE  wee_codigo = 'PD')
    BEGIN
        INSERT  INTO workflow_estado_etapa (
            wee_codigo,
            wee_nombre
        )
        VALUES                            ('PD', 'Pendiente'),
...
IF NOT EXISTS (SELECT 1
               ...
               WHERE  TABLE_NAME = 'solicitudes'
                      AND COLUMN_NAME = 'sol_wet_id')
    BEGIN
        ALTER TABLE solicitudes
            ADD sol_wet_id INT NULL,
                sol_wee_id INT NULL;
        ALTER TABLE solicitudes
            ADD CONSTRAINT fk_solicitudes_etapa FOREIGN KEY (sol_wet_id) REFERENCES workflow_etapas (wet_id);
        ALTER TABLE solicitudes
            ADD CONSTRAINT fk_solicitudes_resultado FOREIGN KEY (sol_wee_id) REFERENCES workflow_estado_etapa (wee_id);
        CREATE INDEX idx_solicitudes_etapa_actual
            ON solicitudes(sol_wet_id);
        CREATE INDEX idx_solicitudes_resultado_etapa
            ON solicitudes(sol_wee_id);
    END
...
-- ============================================================================
DECLARE @cen_id AS INT = (SELECT wet_id
                          FROM   workflow_etapas
                          WHERE  wet_codigo = 'EJN');

DECLARE @sac_id AS INT = (SELECT wet_id
                          FROM   workflow_etapas
                          WHERE  wet_codigo = 'ASC');

DECLARE @pd_id AS INT = (SELECT wee_id
                         FROM   workflow_estado_etapa
                         WHERE  wee_codigo = 'PD');

DECLARE @ap_id AS INT = (SELECT wee_id
                         FROM   workflow_estado_etapa
                         WHERE  wee_codigo = 'AP');

DECLARE @rz_id AS INT = (SELECT wee_id
                         FROM   workflow_estado_etapa
                         WHERE  wee_codigo = 'RZ');

-- Solo actualizar registros que aún no han sido mapeados
UPDATE solicitudes
SET    sol_wet_id = @cen_id,
       sol_wee_id = @pd_id
WHERE  sol_ses_id = 1
       AND sol_wet_id IS NULL;

UPDATE solicitudes
SET    sol_wet_id = @sac_id,
       sol_wee_id = @pd_id
WHERE  sol_ses_id = 2
       AND sol_wet_id IS NULL;
...
PRINT '✅ Nuevas columnas agregadas a solicitudes: sol_wet_id, sol_wee_id';
